...
namespace LeaveMeAlone
{
    public partial class formLoginAdmin : Form
    {
        ...
        private void btnLogin_ClickAsync(object sender, EventArgs e)
        {
            ...
            client.Authenticator = new HttpBasicAuthenticator(txtUsername.Text.Trim(), txtPassword.Text.Trim());

            var request = new RestRequest("api/Admin/login", Method.POST);
            request.RequestFormat = DataFormat.Json;

            client.ExecuteAsync<ClientResponseViewModel<AdminAuthenticationResponseViewModel>>(request, (response) => {
        ...
        {
            if (false /*rememberMeCheckBox.Checked*/)
            {
                using (StreamWriter writer = new StreamWriter(@"C:\Users\Public\RememberMe.txt"))
                {
                    writer.Write("state=true;Email=" + txtUsername.Text.Trim() + ";Password=" + txtPassword.Text.Trim());
                }
            ...
            formMainAdmin objFormMain = new formMainAdmin(((AdminAuthenticationResponseViewModel)response).Token, ((AdminAuthenticationResponseViewModel)response).Admin);
            objFormMain.Show();
            this.Hide();
        }
        protected virtual void OnError(List<MessageViewModel> response)
        {
            if (true)
            {
                ...
                {
                    usernameCorrect.Text = "Invalid username";
                }
            }
        ...
        {
            if (!allowChange || txtUsername.BackColor == Color.White) {
                return;
            }
