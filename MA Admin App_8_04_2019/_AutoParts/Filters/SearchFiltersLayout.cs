...
{
    public partial class SearchFiltersLayout : UserControl
    {

        private FilterViewFormAdmin filterViewForm;

        private bool first = true;

        public SearchFiltersLayout()
        {
            InitializeComponent();
        ...
        private void btnApplyFilters_Click(object sender, EventArgs e) {
            ...
            foreach (var v in dataFilterType.CheckedItems) {
                technicalDescription += ",Vrsta filtra:" + v;
            }

            //getting all selected values from izkoristek goriva
            foreach (var v in dataFilterShape.CheckedItems) {
                technicalDescription += ",Oblika filtra:" + v;
            }
            ...
            formMainAdmin.mainForm.FunctionSummoner(41, category:"Filter", technicalDescription: technicalDescription, priceMin: priceMin, priceMax: priceMax);
        }
                ...
                if (list.GetItemRectangle(i).Contains(list.PointToClient(MousePosition))) {
                    switch (list.GetItemCheckState(i)) {
                        case CheckState.Checked:
                            ...
                            if (list.GetItemText(list.Items[i]).Equals("Okrogel")) {
                                txtDiameter.Text = "";
        ...
        private void tiresData_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e) {
            ...
            filterViewForm = new FilterViewFormAdmin(autoPart);
            filterViewForm.Show();
        }
    }
}
